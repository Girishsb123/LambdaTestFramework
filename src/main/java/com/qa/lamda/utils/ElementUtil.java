...
public class ElementUtil {
	...
	@Step("clicking on element : {0} with timeout {1}")
	public void doClickWithWait(By locator, int timeOut) {
		waitForVisibilityOfElement(locator, timeOut).click();
	}

	@Step("entering value {1} to element : {0} with timeout {2}")
	 ...
	 * @return
	 */
	@Step("waiting for elements : {0} with timeout {1}")
	...
	@Step("waiting for page title : {0} with timeout {1}")
	...
	@Step("waiting for frame by locator : {0} with timeout {1}")
	public void waitForFrameByLocator(By frameLocator, int timeOut) {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeOut));
		wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(frameLocator));
	}

	@Step("waiting for frame by index : {0} with timeout {1}")
	public void waitForFrameByIndex(int frameIndex, int timeOut) {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeOut));
		wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(frameIndex));
	}

	@Step("waiting for frame by IDOrName : {0} with timeout {1}")
	public void waitForFrameByIDOrName(String IDOrName, int timeOut) {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeOut));
		wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(IDOrName));
	}

	@Step("waiting for frame by element : {0} with timeout {1}")
	public void waitForFrameByElement(WebElement frameElement, int timeOut) {
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeOut));
		wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(frameElement));
	}

	@Step("waiting for windows exist : {1} with timeout {0}")
	...
	@Step("click on element when it is ready : {0} with timeout {1}")
	...
	@Step("waiting for element with fluent wait : {0} with timeout {1}")
